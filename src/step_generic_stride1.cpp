...
/* These macros get into the guts of the PS1LOOP_OVER_VOL loops to
...
void step_curl_stride1(RPR f, component c, const RPR g1, const RPR g2, ptrdiff_t s1,
                       ...
                       direction dsigu, const RPR sigu, const RPR kapu, const RPR siginvu,
                       realnum dt, const RPR cnd, const RPR cndinv, RPR fcnd) {
    ...
    if (dsigu == NO_DIRECTION) { // no fu update
      if (cnd) {
        ...
        if (g2) {
          ...
          PS1LOOP_OVER_IVECS(gv, is, ie, i) { f[i] -= dtdx * (g1[i + s1] - g1[i]); }
        }
      }
    }
...
void step_beta_stride1(RPR f, component c, const RPR g, const grid_volume &gv, const ivec is,
                       const ivec ie, realnum betadt, direction dsig, const RPR siginv, RPR fu,
                       direction dsigu, const RPR siginvu, const RPR cndinv, RPR fcnd) {
  (void)c; // currently unused
  if (!g) return;
  if (dsig != NO_DIRECTION) { // PML in f update
    ...
    if (dsigu != NO_DIRECTION) { // fu, no PML in f
      ...
      if (cndinv) { // conductivity, no PML
        PS1LOOP_OVER_IVECS(gv, is, ie, i) { f[i] += betadt * g[i] * cndinv[i]; }
      }
      else { // no conductivity or PML
        PS1LOOP_OVER_IVECS(gv, is, ie, i) { f[i] += betadt * g[i]; }
      }
    }
  }
}
...
void step_update_EDHB_stride1(RPR f, component fc, const grid_volume &gv, const ivec is,
                              const ivec ie, const RPR g, const RPR g1, const RPR g2, const RPR u,
                              const RPR u1, const RPR u2, ptrdiff_t s, ptrdiff_t s1, ptrdiff_t s2,
                              const RPR chi2, const RPR chi3, RPR fw, direction dsigw,
                              const RPR sigw, const RPR kapw) {
  ...
  if (dsigw != NO_DIRECTION) { //////// PML case (with fw) /////////////
    ...
    if (u1 && u2) { // 3x3 off-diagonal u
      ...
      if (chi3) {
        ...
        PS1LOOP_OVER_IVECS(gv, is, ie, i) { f[i] = g[i]; }
    }
  }
}
